...
to detect-communities
  ...
  foreach ( range 0 length( agentsetlouv)) [
    ...
    set list-communities-mean insert-item 0 list-communities-mean mean-com
    set list-communities-std insert-item 0 list-communities-std std-com
...
to communicate-via [ algorithm ] ;; node procedure
  ...
  if (  ( algorithm = "bayesianSAM" ) or ( algorithm = "bayesianMAP" )) [

    ;; speak according to Bayesian algorithm
    ifelse (algorithm = "bayesianSAM") [ speak-bayesianSAM ] [ speak-bayesianMAP ]
...
to-report ratio-cen
  report ( mean [ initial-centrality ] of turtles with [ (state = 1) or (state = init-langval-1) ] ) / ( mean [ initial-centrality ] of turtles with [ (state = 0) or (state = init-langval-0) ] )
end

;; the connection probability
to-report con-prob
  set nb-neighb []
  ask turtles [ set nb-neighb insert-item 0 nb-neighb count my-out-links ]
  report mean nb-neighb / num-nodes
end


;; the mean language value of turtle
to-report mean-langval
 report mean [ language-value ] of turtles
end

;; the mean language value of biased turtles
to-report mean-langval-1
 ifelse any? turtles with [state = init-langval-1]
  [ report mean [ language-value ] of turtles with [ state = init-langval-1 ] ]
  [ report "NA" ]
end
...
to-report mean-langval-0
  ...
  [ report mean [ language-value ] of turtles with [ state = init-langval-0 ] ]
  [ report "NA" ]
end
...
update-algorithm
update-algorithm
"individual" "threshold" "reward" "probabilistic" "bayesianSAM" "bayesianMAP"
4
...
@#$#@#$#@
<experiments>
  <experiment name="trial" repetitions="10" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="N-influent">
      <value value="0"/>
    </enumeratedValueSet>
  </experiment>
